...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const SELLER_ORDERS_KEY = "orders_seller";

  // Đồng bộ trạng thái từ SellerHub về đơn gốc của khách (key 'orders')
  function updateCustomerOrderStatus(sourceId, sellerStatus) {
    if (!sourceId) return;
    try {
      const raw = localStorage.getItem("orders");
      if (!raw) return;
      const customerOrders = JSON.parse(raw);
      if (!Array.isArray(customerOrders)) return;

      const idx = customerOrders.findIndex((o) => o.id === sourceId);
      if (idx === -1) return;
      ...
      const newStatus = statusMap[sellerStatus] || "pending";
      const statusTextMap = {
      ...
      const order = customerOrders[idx];
      order.status = newStatus;
      order.statusText = statusTextMap[newStatus] || "Chờ xác nhận";

      // Cập nhật timeline cơ bản nếu có
      if (Array.isArray(order.timeline) && order.timeline.length) {
        const now = new Date().toLocaleString("vi-VN");
        const markStep = (label) => {
          const step = order.timeline.find((s) => s.status && s.status.includes(label));
          if (step) {
            step.completed = true;
            if (!step.time) step.time = now;
          }
        };

        if (newStatus === "processing") markStep("Đang xử lý");
        if (newStatus === "shipping") markStep("Đang giao");
        if (newStatus === "delivered") markStep("Đã giao");
        if (newStatus === "cancelled") markStep("Đã hủy");
      }

      localStorage.setItem("orders", JSON.stringify(customerOrders));
    } catch (e) {
      console.error("Lỗi khi đồng bộ trạng thái đơn hàng khách:", e);
    }
  }

  // Load đơn hàng cho SellerHub từ localStorage
  function loadOrdersFromStorage() {
    // Đọc dữ liệu SellerHub hiện tại (nếu có)
    let sellerOrders = [];
    const sellerRaw = localStorage.getItem(SELLER_ORDERS_KEY);
    if (sellerRaw) {
      try {
        const parsed = JSON.parse(sellerRaw);
        if (Array.isArray(parsed)) sellerOrders = parsed;
      } catch (e) {
        console.error("Lỗi khi đọc orders_seller từ localStorage:", e);
      }
    }

    // Luôn đọc thêm từ 'orders' (khách) để lấy các đơn mới nếu có
    ...
    if (stored) {
      try {
        ...
        const isCustomerOrderFormat = (o) => {
          return o && !o.code && o.id && Array.isArray(o.items);
        };

        if (parsed.some(isCustomerOrderFormat)) {
          // Map nhanh để kiểm tra đơn đã được import sang SellerHub chưa
          const existingSourceIds = new Set(
            sellerOrders.filter(o => o && o.sourceId).map(o => o.sourceId)
          );

          const newConverted = parsed
            .filter(o => isCustomerOrderFormat(o) && !existingSourceIds.has(o.id))
            .map((o) => {
              const shippingInfo = o.shippingInfo || {};

              const name = shippingInfo.fullname || "Khách lẻ";
              const avatar =
                name
                  .split(" ")
                  .filter(Boolean)
              ...
              const total =
                typeof o.total === "number"
                  ? o.total
                  : (o.subtotal || 0) + (o.shipping || 0) - (o.discount || 0);

              const paymentMethod = o.paymentMethod || "cod";
              const paymentMeta =
                {
                  cod: { method: "COD", paid: false },
                  bank: { method: "Banking", paid: true },
                  momo: { method: "Momo", paid: true },
                  vnpay: { method: "VNPay", paid: true },
                }[paymentMethod.toLowerCase()] || { method: paymentMethod, paid: false };
          ...
          if (newConverted.length > 0) {
            sellerOrders = sellerOrders.concat(newConverted);
            try {
              localStorage.setItem(SELLER_ORDERS_KEY, JSON.stringify(sellerOrders));
            } catch (e) {
              console.error("Lỗi khi lưu orders_seller sau khi merge:", e);
            }
          }
        }
      } catch (e) {
    ...
    return sellerOrders.length ? sellerOrders : null;
  }

  // Lưu đơn hàng SellerHub vào localStorage (key riêng)
  function saveOrdersToStorage(orders) {
    try {
      localStorage.setItem(SELLER_ORDERS_KEY, JSON.stringify(orders));
    } catch (e) {
      console.error("Lỗi khi lưu orders_seller vào localStorage:", e);
    }
  }
    ...
    if (actionItem) {
      ...
      if (action === "setStatus") {
        ...
        if (idx >= 0) {
          ...
          saveOrdersToStorage(ordersSeed);
          updateCustomerOrderStatus(ordersSeed[idx].sourceId, ordersSeed[idx].status);
        }
        state.openMenuCode = null;
        render();
        return;
